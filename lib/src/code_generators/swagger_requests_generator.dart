        ...
        final hasOptionalBody =
            ['post', 'put', 'patch'].contains(requestType) &&
                swaggerRequest.parameters
                    .none((p) => p.inParameter == _options.requestBodyName);
  ...
  List<Parameter> _getAllParameters({
    ...
    if (requestBody != null) {
      ...
      if (requestBody.content?.isMultipart == true) {
        ...
        // in case a scheme for the request is defined, we use only one param const _options.requestBodyName and the type of this param will be the scheme as class.
        if (requestBody.content?.schema?.ref.isNotEmpty == true) {
          result.add(
            Parameter(
              (p) => p
                ..name = _options.requestBodyName
                ..named = true
                ..required = true
